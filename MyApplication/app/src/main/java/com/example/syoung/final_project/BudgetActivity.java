...
public class BudgetActivity extends Activity implements AdapterView.OnItemClickListener{
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if("Net Income".equals(((TextView) view).getText())){
           final Dialog incomeDialog = new Dialog(context);
            ...
            incomeDialog.setTitle("Net Income");
            setUpIncomeDialogComponents(incomeDialog);

        }
        else if("Add Bill".equals(((TextView) view).getText())){
            Intent addBill = new Intent(getApplicationContext(), FindItemActivity.class);
            addBill.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            addBill.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(addBill);
        }
        else if("View/Edit Bill".equals(((TextView) view).getText())){
            Intent editViewBill = new Intent(getApplicationContext(), FindItemActivity.class);
    ...
    private void setUpIncomeDialogComponents(final Dialog incomeDialog){
        income_text = (EditText) incomeDialog.findViewById(R.id.netincome);
        income_text.setInputType(InputType.TYPE_CLASS_NUMBER);
        ...
        income_text.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void afterTextChanged(Editable s) {
                ...
                if(!s.toString().matches("^\\$(\\d{1,3}(\\,\\d{3})*|(\\d+))(\\.\\d{2})?$")){
                    ...
                    income_text.removeTextChangedListener(this);
                    income_text.setText(netIncomeSB.toString());
                    income_text.setTextKeepState("$" + netIncomeSB.toString());
                    Selection.setSelection(income_text.getText(), netIncomeSB.toString().length() + 1);
                    income_text.addTextChangedListener(this);
        ...
        income_submit_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!"".equals(income_text.getText().toString())) {
                    Double amount = Double.parseDouble(income_text.getText().toString().replace("$" ,""));
                    netIncomeAmount.setIncome(amount);
                    saveIncome();
                    incomeDialog.dismiss();
                }
                else {
                    Toast.makeText(getApplicationContext(), "please add your income",
                            Toast.LENGTH_SHORT).show();
                }
            }
        });
